...
def parse_water_dat(path):
    ...
    with open(path) as f:
        for line in f:
            ...
            if not line or line.lower().startswith('processed') or line.startswith('#'): continue
            parts=re.split(r'[\s,]+', line)
            parts=[p for p in parts if p]
            if len(parts) not in (22,29): continue
...
def ensure_material(name,color):
    ...
    if not mat:
        ...
        mat.diffuse_color=color; mat.use_nodes=False; mat.blend_method='BLEND'
    return mat
...
def color_code_collection(collection,threshold):
    ...
    for obj in collection.objects:
        if not getattr(obj,'is_water',False): continue
...
class IMPORT_OT_water_dat(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if not faces: self.report({'ERROR'},"No entries"); return{'CANCELLED'}
        colname='WaterIO'
        ...
        col=bpy.data.collections.new(colname); context.scene.collection.children.link(col)
        for i,(coords,params) in enumerate(faces):
            mesh=bpy.data.meshes.new(f'Zone_{i}'); obj=bpy.data.objects.new(f'Zone_{i}',mesh)
            ...
            for p in params: v=obj.water_verts.add(); v.params=p
...
class EXPORT_OT_water_dat(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        if not zones: self.report({'ERROR'},"Select water objects"); return{'CANCELLED'}
        lines=['processed']
        for o in zones:
...
class VIEW3D_PT_water_io(bpy.types.Panel):
    bl_label="Water IO"; bl_idname="VIEW3D_PT_water_io"
    bl_space_type='VIEW_3D'; bl_region_type='UI'; bl_category='Water IO'
...
class OBJECT_PT_water_params(bpy.types.Panel):
    bl_label="Water Params"; bl_idname="OBJECT_PT_water_params"
    bl_space_type='PROPERTIES'; bl_region_type='WINDOW'; bl_context='object'
    @classmethod
    def poll(cls,context): return getattr(context.object,'is_water',False)
    def draw(self,context):
        obj=context.object; layout=self.layout
        layout.prop(obj,'is_water')
        for i,v in enumerate(obj.water_verts): layout.prop(v,'params',text=f"V{i+1}")

class SCENE_PT_water_opts(bpy.types.Panel):
    bl_label="Water Options"; bl_idname="SCENE_PT_water_opts"
...
classes=[WaterVertex,IMPORT_OT_water_dat,EXPORT_OT_water_dat,VIEW3D_PT_water_io,OBJECT_PT_water_params,SCENE_PT_water_opts]
...
def unregister():
    ...
    del bpy.types.Object.is_water; del bpy.types.Object.water_verts; del bpy.types.Scene.shallow_threshold
